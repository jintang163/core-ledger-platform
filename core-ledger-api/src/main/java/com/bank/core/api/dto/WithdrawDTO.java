 ...
 */
@Data
@ApiModel(value = "提现请求", description = "单账户提现（出金）请求参数")
public class WithdrawDTO implements Serializable {
    ...
    @ApiModelProperty(value = "渠道编码", required = true, example = "ALIPAY",
            notes = "提现渠道编码：ALIPAY（支付宝）、WECHAT（微信支付）、UNIONPAY（银联）、MOCK（模拟渠道）")
    ...
    /** 回调地址（渠道处理完成后的回调通知地址） */
    @ApiModelProperty(value = "回调地址", example = "https://api.example.com/callback/withdraw",
            notes = "渠道处理完成后的回调通知地址")
    private String callbackUrl;

    /** 备注 */
    @ApiModelProperty(value = "备注", example = "用户提现",
            notes = "业务备注信息")
