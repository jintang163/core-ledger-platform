 ...
 *
 * 核心功能：
 * 1. 缓冲记账：高并发场景下先记录流水，后异步批量更新余额
 * 2. 流量削峰：通过异步处理将瞬时高并发流量削平
 * 3. 幂等支持：通过requestId和businessNo实现重复请求幂等
 * 4. 失败重试：处理失败的流水支持自动/手动重试
 *
 * 适用场景：
 * - 允许短暂数据不一致（最多延迟5秒）
 * - 高并发批量操作（如批量代发、批量扣款）
 ...
 * 2. 立即返回成功给调用方，降低接口响应时间
 * 3. 后台定时任务（默认每秒1次）批量处理流水，更新账户余额
 * 4. 处理完成后更新流水状态为成功或失败
 * 5. 失败流水根据重试次数决定是否重新进入待处理状态
 *
 * 设计要点：
 * - 金额同时存储元（BigDecimal）和分（Long）两种格式
 * - 状态流转：待处理(0) -> 处理中(1) -> 成功(2)/失败(3)
 * - 最大重试次数：3次，超过后标记为失败需人工介入
 */
@Data
